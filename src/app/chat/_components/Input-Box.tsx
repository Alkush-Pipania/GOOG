...
"use client";
import { ArrowUp, Square, AlertCircle, Brain, HelpCircle, MessageSquare, X } from "lucide-react";
...
const formSchema = z.object({
  ...
  promptType: z.union([z.literal("normal"), z.literal("detailed")]),
});
...
const disclaimerMessages = [
  "GOOG might make mistakes",
  "GOOG is not conversational , only create prompts for you.",
  "it's important to review and adjust suggestions for your specific use case.",
];

const InputBox = ({ isMobile, id }: { isMobile: boolean; id: string }) => {
  const [deepThink, setDeepThink] = useState<boolean>(false);
  const [showDisclaimer, setShowDisclaimer] = useState<boolean>(true);
  const [currentDisclaimer, setCurrentDisclaimer] = useState(0);
  ...
  // Change disclaimer message every 5 seconds
  React.useEffect(() => {
    if (!showDisclaimer) return;
    
    const interval = setInterval(() => {
      setCurrentDisclaimer((prev) => (prev + 1) % disclaimerMessages.length);
    }, 5000);
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      while (true) {
        ...
        updateAssistantMessageStream(chunk);
      }
  ...
  return (
    <>
      {/* Disclaimer */}
      {showDisclaimer && (
        <div className="w-full max-w-4xl mx-auto px-2 sm:px-4 mb-2 sm:mb-4 relative z-[60]">
          <div className="bg-black/30 rounded-lg p-2 sm:p-3 backdrop-blur-sm border border-gray-700/50 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-4">
            <div className="flex items-start sm:items-center gap-1.5 sm:gap-2 w-full sm:w-auto">
              <AlertCircle className="w-4 h-4 sm:w-5 sm:h-5 text-yellow-400 flex-shrink-0 mt-0.5 sm:mt-0" />
            ...
            <button
              ...
              className="p-1 hover:bg-gray-700/50 rounded-full transition-colors self-end sm:self-auto"
            >
                ...
                <div className="flex items-center justify-between mt-2 flex-wrap gap-2">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    >
                      ...
                      <span className={`text-sm font-medium ${isMobile ? "hidden sm:inline" : ""}`}>CodeCraft</span>
                    </button>
                  </div>
